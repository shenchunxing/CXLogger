...
BOOL   CXAPILoggerEnabled         = YES;
char  *CXAPILoggerRequestLogIcon  = "✈️";
char  *CXAPILoggerResponseLogIcon = "☀️";
char  *CXAPILoggerErrorLogIcon    = "❌";

static void * CXAPILoggerTakeOffDate = &CXAPILoggerTakeOffDate;

static dispatch_queue_t _ycJsonResponseFormatQueue;

//分割符
static const char* CXAPILoggerMarkedLine(char* c, uint length) {
...
@implementation CXAPILogger

+ (instancetype)sharedInstance {
    
    static CXAPILogger *logger = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        logger = [[self alloc] init];
        _ycJsonResponseFormatQueue = dispatch_queue_create("CXAPILogger.loggerQueue", DISPATCH_QUEUE_CONCURRENT);
    });
    return logger;
}
...
- (instancetype)init {
    ...
    if (self = [super init]) {
        ...
        self.requestLoggerElements = CXAPILoggerRequestElementAll;
        self.responseLoggerElements = CXAPILoggerResponseElementAll;
    }
    return self;
}
...
- (void)apiDidTakeOff:(NSNotification *)notification {
    ...
    //记录请求开始时间
    objc_setAssociatedObject(notification.object, CXAPILoggerTakeOffDate, [NSDate date], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    NSMutableString *frmtString = @"".mutableCopy;
    //记录请求taskIdentifier
    if (self.requestLoggerElements & CXAPILoggerRequestElementTaskIdentifier) {
    ...
    //请求起飞时间
    if (self.requestLoggerElements & CXAPILoggerRequestElementTakeOffTime) {
        NSDateFormatter * df = [[NSDateFormatter alloc] init];
        [df setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
        NSString *timeStr = [df stringFromDate:objc_getAssociatedObject(notification.object, CXAPILoggerTakeOffDate)];
        NSString *milestoneTimeDes = [NSString stringWithFormat:@"\n<起飞时间>  %@", timeStr];
        [frmtString appendString:milestoneTimeDes];
    }
    //请求方法
    if (self.requestLoggerElements & CXAPILoggerRequestElementMethod) {
        NSString *methodDes = [NSString stringWithFormat:@"\n<请求方式>  %@", request.HTTPMethod];
        [frmtString appendString:methodDes];
    }
    //请求地址
    if (self.requestLoggerElements & CXAPILoggerRequestElementVaildURL) {
        NSString *validURLDes = [NSString stringWithFormat:@"\n<请求地址>  %@", [request.URL absoluteString]];
        [frmtString appendString:validURLDes];
    }
    //请求头
    if (self.requestLoggerElements & CXAPILoggerRequestElementHeaderFields) {
    ...
    //请求体，一般是请求参数
    if (self.requestLoggerElements & CXAPILoggerRequestElementHTTPBody) {
...
- (void)apiDidLand:(NSNotification *)notification {
    ...
    if (self.responseLoggerElements & CXAPILoggerResponseElementTaskIdentifier) {
        NSString *taskIdentifier = [self taskIdentifierFromAFNotification:notification];
        NSString *taskIdentifierDes = [NSString stringWithFormat:@"\n<API序列号> %@", taskIdentifier];
        [frmtString appendString:taskIdentifierDes];
    }
    ...
    if (self.responseLoggerElements & CXAPILoggerResponseElementLandTime) {
        NSDateFormatter * df = [[NSDateFormatter alloc] init];
        [df setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    ...
    if (self.responseLoggerElements & CXAPILoggerResponseElementTimeConsuming) {
        NSTimeInterval timeConsuming = [landDate timeIntervalSinceDate:objc_getAssociatedObject(notification.object, CXAPILoggerTakeOffDate)];
    ...
    if (self.responseLoggerElements & CXAPILoggerResponseElementMethod) {
        NSString *methodDes = [NSString stringWithFormat:@"\n<请求方式>  %@", request.HTTPMethod];
        [frmtString appendString:methodDes];
    }
    
    if (self.responseLoggerElements & CXAPILoggerResponseElementStatusCode) {
    ...
    if (self.responseLoggerElements & CXAPILoggerResponseElementVaildURL) {
        NSString *validURLDes = [NSString stringWithFormat:@"\n<请求地址>  %@", [request.URL absoluteString]];
        [frmtString appendString:validURLDes];
    }
    
    if (self.responseLoggerElements & CXAPILoggerResponseElementHeaderFields) {
    ...
    if (self.responseLoggerElements & CXAPILoggerResponseElementResponse) {
        if (error) {
...
- (void)showRequestLogger:(NSString *)fmrtStr {
    ...
    printf("\n%s", CXAPILoggerMarkedLine(CXAPILoggerRequestLogIcon, 16));
    printf("  ← 请求日志 →  ");
    printf("%s\n", CXAPILoggerMarkedLine(CXAPILoggerRequestLogIcon, 16));
    printf("%s\n", [fmrtStr UTF8String]);
    printf("\n%s\n", CXAPILoggerMarkedLine(CXAPILoggerRequestLogIcon, 40));
}

- (void)showResponseLogger:(NSString *)fmrtStr {
    
    printf("\n%s", CXAPILoggerMarkedLine(CXAPILoggerResponseLogIcon, 16));
    printf("  ← 响应日志 →  ");
    printf("%s\n", CXAPILoggerMarkedLine(CXAPILoggerResponseLogIcon, 16));
    printf("%s\n", [fmrtStr UTF8String]);
    printf("\n%s\n", CXAPILoggerMarkedLine(CXAPILoggerResponseLogIcon, 40));
}

- (void)showErrorLogger:(NSString *)fmrtStr {
    
    printf("\n%s", CXAPILoggerMarkedLine(CXAPILoggerErrorLogIcon, 16));
    printf("  ← 异常日志 →  ");
    printf("%s\n", CXAPILoggerMarkedLine(CXAPILoggerErrorLogIcon, 16));
    printf("%s\n", [fmrtStr UTF8String]);
    printf("\n%s\n", CXAPILoggerMarkedLine(CXAPILoggerErrorLogIcon, 40));
}
